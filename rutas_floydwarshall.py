...
Autores: Catalina Calderón, Fátima Torres, Jack Vera, Danna Zambrano
"""
...
class DescargadorRed:
    ...
    def __init__(self):
        ...
        self.bbox_guayaquil = [ -2.250  ,-79.970 , -2.000,  -79.850 ]  # Ceibos+ESPOL box (forced)
...
"""
    ...
    def procesar_red_osm(self, data_osm: Dict) -> Dict:
        ...
        MAX_NODOS = 20000  # límite duro de nodos en la red procesada
        ...
        for k in range(n):
            ...
            for i in range(n):
                if self.matriz_distancias[i][k] == INF:
                    continue
                for j in range(n):
                    nueva_distancia = (self.matriz_distancias[i][k] + self.matriz_distancias[k][j])
...
class InterfazSistemaEmergencia:
    ...
    def _crear_seccion_ubicacion(self, parent):
        ...
        tk.Label(frame,
                text="Ejemplos: ESPOL, Malecón 2000, Mall del Sol, Los Ceibos, Urdesa, etc.",
